...
export const TicketOutputPreview: React.FC<TicketOutputPreviewProps> = ({ pages, onClose, customFonts, documentId }) => {
  ...
  const handleAssignToUser = async () => {
    ...
    try {
      ...
      // 1) Build layout JSON compatible with /api/generate-output-pdf
      // We mirror the same 3-cars-per-page layout used in this preview.
      const ticketHeightPx = (A4_HEIGHT_PX - A4_FOOTER_PX) / 4;
      const layoutPages = pages.map((page) => ({
        items: page.tickets.map((_, idx) => ({
          type: 'image' as const,
          src: page.ticketImageData, // already composited ticket (car + series)
          x: 0,
          y: ticketHeightPx * idx,
          width: A4_WIDTH_PX,
          height: ticketHeightPx,
        })),
      }));

      // 2) Ask backend to generate and store a real output PDF for this layout
      const genRes = await fetch('http://localhost:4000/api/generate-output-pdf', {
        method: 'POST',
        headers: {
      ...
      if (!genRes.ok) {
        const message = genData.message || 'Failed to generate output PDF';
        throw new Error(message);
      }

      const outputDocumentId = genData.documentId;
      if (!outputDocumentId) {
        throw new Error('Generated documentId missing from response');
      }

      // 3) Assign this generated-output document to the target user
      const res = await fetch(`http://localhost:4000/api/admin/documents/${outputDocumentId}/assign-by-email`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ email: assignEmail, assignedQuota: pagesNum }),
      });
        ...
        (id) => (
          ...
          >
            ...
            <span className="text-foreground">
              {pagesNum} pages assigned to <span className="font-medium">{assignEmail}</span>.
            </span>
            <span className="text-[11px] text-muted-foreground">
              User apne account se login karke <span className="font-semibold">/printing</span> page par prints dekh sakta hai.
            </span>
          </div>
        ),
        { duration: 4000 }
