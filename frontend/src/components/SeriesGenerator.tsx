...
export const SeriesGenerator = ({ userId }: SeriesGeneratorProps) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Call edge function to generate series
      const { data, error } = await supabase.functions.invoke('generate-series-pdf', {
        body: {
          templateBase64,
          templateType: templateFile.type === 'application/pdf' ? 'pdf' : 'svg',
          startNumber,
          endNumber,
          userId
        }
