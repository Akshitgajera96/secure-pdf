...
const Security = () => {
  ...
  const loadHistory = async (pageNum: number) => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/security/login-history?page=${pageNum}&limit=20`,
        { headers: authHeaders() }
      );
      const data = await res.json();
      if (!res.ok || !data.success) {
  ...
  const handleAddIp = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/security/ip-whitelist`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ ip: newIp.trim(), action: "add" }),
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
  ...
  const handleToggleWhitelist = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/security/toggle-ip-whitelist`, {
        method: "POST",
        headers: authHeaders(),
